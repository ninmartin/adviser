...
# create domain
pokedex_domain = JSONLookupDomain(name="pokedex")

# create domain specific modules
nlu = HandcraftedNLU(domain=pokedex_domain)
bst = HandcraftedBST(domain=pokedex_domain)
policy = HandcraftedPolicy(domain=pokedex_domain)
nlg = HandcraftedNLG(domain=pokedex_domain)
d_tracker = DomainTracker(domains=[pokedex_domain])
